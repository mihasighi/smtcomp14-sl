...
(define-fun TLL_plus_rev ((?top TLL_t) (?p TLL_t) (?mleft TLL_t) (?mright TLL_t)) Space
  (tospace (or (and (pto ?top (sref (ref left nil) (ref right nil) (ref parent ?p) (ref next ?mright))) (= ?top ?mleft))
  	       (exists ((?x TLL_t) (?up TLL_t) (?lr TLL_t)) 
	       	       (and (tobool (ssep (pto ?x (sref (ref left nil) (ref right nil) (ref parent ?up) (ref next ?lr)))
		       	    	    	  (TTL_aux ?up ?p ?lr ?x ?top ?mright)))
			    (= ?x ?mleft)))
)))
