...
module Term =
  struct
    ...
    let to_smtlib v = if v=nil then keyw_nil.str else "?" ^(Var.to_string v)
    let listref_to_smtlib l = 
	(Blist.count_foldl (fun ls n v -> ls ^ " (ref f" ^ (string_of_int  n) ^ " " ^ (to_smtlib v) ^ ") ") "" l)
    let list_to_smtlib l = Blist.to_string " " to_smtlib l
...
let pair_to_smtlib sep p =
  let (x,y) = Pair.map Term.to_smtlib p in x ^ sep ^ y
...
module UF =
  struct
    ...
    let to_smtlib v = 
	Blist.map (fun p -> "(= " ^ (pair_to_smtlib " " p) ^ ")" ) (bindings v)
    let get_locals v locs pars = 
	let pvars = (bindings v) in
	let is_in x lenv genv = 
		let lx = Blist.find_first (Term.equal x) lenv in
		let gx = Blist.find_first (Term.equal x) genv in
		match lx, gx with
		| None, None -> if (Term.is_nil x) then [] else [x]
		| _, _ -> []
	in
	(Blist.fold_left (fun acc (x,y) -> acc@(is_in x locs pars)@(is_in y locs pars)) locs pvars)
...
module Deqs =
  struct
    ...
    let to_smtlib v = 
	Blist.map (fun p -> "(distinct " ^ (pair_to_smtlib " " p) ^ ")" ) (elements v)
    let get_locals v locs pars = 
	let pvars = (elements v) in
	let is_in x lenv genv = 
		let lx = Blist.find_first (Term.equal x) lenv in
		let gx = Blist.find_first (Term.equal x) genv in
		match lx, gx with
...
module Ptos =
  struct
    ...
    let elem_to_smtlib info_c (x,args) =
	let _ = (Printf.fprintf info_c "\n;;pto %d\n" (List.length args)) in
	let sref = (Term.listref_to_smtlib args) in
	"(pto " ^ (Term.to_smtlib x) ^ " " ^
	(match args with a::[] -> sref | _ -> "(sref " ^ sref ^ ")") ^
	")"
    ...
    let to_smtlib info_c v = (Blist.map (elem_to_smtlib info_c) (elements v))
    let get_locals v locs pars = 
	let pvars = (elements v) in
	let is_in x lenv genv = 
		let lx = Blist.find_first (Term.equal x) lenv in
		...
		| _, _ -> []
	in
	let collect = fun genv acc x -> acc@(is_in x acc genv) in 
	(Blist.fold_left 
	   (fun acc (x,args) -> 
              let nenv = (Blist.fold_left (collect pars) acc args) in 
		nenv@(is_in x nenv pars)) locs pvars)
...
module Inds =
  struct
    ...
    let elem_to_smtlib (t,(ident,args)) =
      "(" ^ ident ^ " " ^ (Term.list_to_smtlib args) ^ ")"
    ...
    let to_smtlib v = Blist.map elem_to_smtlib (elements v)
    let get_locals v locs pars = 
	let pvars = (elements v) in
	let is_in x lenv genv = 
		let lx = Blist.find_first (Term.equal x) lenv in
	...
	let collect = fun genv acc x -> acc@(is_in x acc genv) in 
	(Blist.fold_left 
	   (fun acc (t,(ident,args)) -> 
              (Blist.fold_left (collect pars) acc args)) locs pvars)
...
module Heap =
  struct
    ...
    let to_smtlib info_c f params = 
      (* collect existential vars *)
      let loceqs = UF.get_locals f.eqs [] params in
      ...
      let septo = Ptos.to_smtlib info_c f.ptos in
      let seprd = Inds.to_smtlib f.inds in
	locals, peq@pdeq, septo@seprd
...
module Defs =
  struct
    ...
    let to_smtlib (d:t) : unit = 
       ...
       * information set-info and for a fixed (5) number of fields,
       *)
      let asrt_c = open_out "assert.smt2" in
      let info_c = open_out "info.smt2" in
      begin 
        (* output sort *)
        (* Warning: use a generic sort *)
        Printf.fprintf asrt_c "\n;generic sort \n";
        Printf.fprintf asrt_c "\n(declare-sort GTyp 0)\n";
	flush asrt_c;

	(* output fields *)
        (* Warning: outputs a fixed number of generic fields *)
        Printf.fprintf asrt_c "\n;generic fields \n";
	Printf.fprintf asrt_c "(declare-fun f0 () (Field GTyp GTyp))\n";
	Printf.fprintf asrt_c "(declare-fun f1 () (Field GTyp GTyp))\n";
	(* post-process depending on the max arity of pto *)

	(* output predicate definitions *)
	(* Warning: the order is the one in the file, 
           which suppose no forward use *)
        (Printf.fprintf asrt_c "\n;predicates \n");
        (Printf.fprintf asrt_c "\n%s\n"
	...
	(* Warning: generate a generic list of vars *)
        Printf.fprintf asrt_c "\n;vars \n";
        (* Printf.fprintf asrt_c "(define-fun x1 () GTyp)\n"; *)
        (* Printf.fprintf asrt_c "(define-fun x2 () GTyp)\n"; *)
	(* post-process depending on the arity of pred *)
